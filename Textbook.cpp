...
//March 19,2023
//Textbook.cpp
//This file implements display() and displayFiltler().
...
*/ 
//We have 4 cases:It does/ not have review questions. It is / not available digitally 
void Textbook::display (){ //Runs!!
if (isDigital() && checkReviewQuestions()){
  std::cout <<getTitle()<< " is written by " << getAuthor()<< " for " << getGradeLevel() << " students." <<" Subject: "  << subject_ << ". "<<  "It has review questions. " << "Page Count: "<<getPageCount() << ". " <<"It is available digitally.\n";
  } else if (!isDigital() && !checkReviewQuestions()){
  std::cout <<getTitle()<< " is written by " << getAuthor()<< " for " << getGradeLevel() << " students." <<" Subject: "  << subject_ << ". "<<  "It does not have review questions. " << "Page Count: "<<getPageCount() << ". " <<"It is not available digitally.\n";
  } else if (!isDigital() && checkReviewQuestions()) {
  std::cout <<getTitle()<< " is written by " << getAuthor()<< " for " << getGradeLevel() << " students." <<" Subject: "  << subject_ << ". "<<  "It has review questions. " << "Page Count: "<<getPageCount() << ". " <<"It is not available digitally.\n";
  } else if (isDigital() && !checkReviewQuestions()){
  std::cout <<getTitle()<< " is written by " << getAuthor()<< " for " << getGradeLevel() << " students." <<" Subject: "  << subject_ << ". "<<  "It does not have review questions. " << "Page Count: "<<getPageCount() << ". " <<"It is available digitally.\n";
  }
}
...
/**
  ...
  */
void Textbook::displayFilter (const std::string& key) { //idk why its not working CHECK!!
  if (subject_ == key){
    display();
  }
}
