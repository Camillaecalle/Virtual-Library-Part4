...
//March 19,2023
//Manual.cpp
//Manual.cpp implements display() and displayFilter(), and edited given parameterized constructor.
#include "Manual.hpp"
 ...
 *////only need two if true is
void Manual::display() { 
  	std::string digital = "It is"; 
    if(!isDigital()) {
      digital = "It is not";  
      } 
      std::cout <<getTitle()<< " is written by " << getAuthor() << " company for device: " << getDevice() << ". Website: " <<getWebsite() << ". Page Count: " << getPageCount() << ". " << digital << " available digitally.\n";
    }
...
void Manual::displayFilter(const std::string& key){
    device_ = device_.substr(0, device_.find('-'));
    if (device_ == key)
    display();
  }
